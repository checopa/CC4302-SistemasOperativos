...
int task_proc(nTask target, int delay, void *msg, void *expected) {
  void *exch_msg;
  nSleep(delay);
...
int test1() {
  ...
  nTask t= nEmitTask(task_proc, nCurrentTask(), 10, &msg1, &msg2);
...
int test2() {
  ...
  nTask t_a= nEmitTask(task_proc, nCurrentTask(), 10, &msg1_a, &msg2_a);
  nTask t_b= nEmitTask(task_proc, nCurrentTask(), 20, &msg1_b, &msg2_b);
...
int test_parallel() {
  ...
  for (i=0; i<NITER; i++) {
    ...
    tasks[3*k+2]= nEmitTask(i%1==0 ? test1 : test2);
    k++;
    if (k==NTASKS)
      k= 0;
  }
...
int nMain(int argc, int argv) {
  ...
  nPrintf("Test 3: Test de robustez con nSetTimeSlice(1)\n");
  test_parallel();
